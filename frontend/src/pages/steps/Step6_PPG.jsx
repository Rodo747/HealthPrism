    ...
    <div className="app-shell" style={{paddingBottom:100}}>
      ...
      <div style={{padding:'20px 20px 0'}}>
        ...
        <div style={{background:'#050C1A',borderRadius:20,height:260,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:14,position:'relative',overflow:'hidden',cursor:phase==='idle'?'pointer':'default'}} onClick={phase==='idle'?startMeasure:undefined}>
           <video ref={videoRef} autoPlay playsInline muted style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',opacity:phase==='measuring'?0.3:0}}/>
           {phase==='measuring'&&<div className="pulse-anim" style={{position:'absolute',inset:0,background:'radial-gradient(circle at center,rgba(167,35,45,.25) 0%,transparent 65%)'}}/>}
           <div style={{position:'relative',zIndex:1,width:72,height:72,borderRadius:'50%',background:phase==='measuring'?'linear-gradient(135deg,#a7232d,#c42b36)':phase==='done'?'linear-gradient(135deg,#10B981,#34D399)':'#1A2540',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:phase==='measuring'?'0 0 30px rgba(167,35,45,.5)':'none',transition:'all .3s'}}>
          ...
          {phase==='measuring'&&(
            <div style={{textAlign:'center',position:'relative',zIndex:1}}>
              ...
              <div style={{width:160,height:4,background:'rgba(255,255,255,.1)',borderRadius:999,marginTop:10,overflow:'hidden'}}>
                 <div style={{width:`${progress}%`,height:'100%',background:'linear-gradient(90deg,#a7232d,#c42b36)',borderRadius:999,transition:'width .2s'}}/>
              </div>
          ...
          {phase==='done'&&bpm&&(
            <div style={{textAlign:'center',position:'relative',zIndex:1}}>
              ...
              <p style={{fontSize:13,color:'#8496A9',marginTop:4}}>BPM</p>
               <div style={{marginTop:10,display:'inline-flex',background:bpm>120?'#FDECEA':'#D1FAE5',color:bpm>120?'#a7232d':'#047857',padding:'4px 14px',borderRadius:999,fontSize:12,fontWeight:700}}>
