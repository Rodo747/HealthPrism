...
import { runHandModel } from '../../ml/modelLoader'
...
const Step4_Hand = () => {
  ...
  const handleCapture = async () => {
    ...
    try {
      const s = await runHandModel(r.imageElement)
  ...
  const scoreLabel = score==null ? null : score>=65 ? {text:'Pale palm/nails detected',color:'#E8291C',bg:'#FDECEA'} : score>=40 ? {text:'Mild pallor detected',color:'#B45309',bg:'#FEF3C7'} : {text:'Normal coloration',color:'#047857',bg:'#D1FAE5'}

  return (
    <div className="app-shell" style={{paddingBottom:100}}>
      <StepHeader step={4} total={10} title="Hand Analysis" onBack={()=>{ stop(); navigate('/check/step3') }}/>
      <div style={{padding:'20px 20px 0'}}>
        ...
        {captured ? (
          ...
          <CameraView videoRef={videoRef} shape="hand" instruction="Open the palm facing up and place it flat inside the guide" onCapture={handleCapture} onFlip={flip} accentColor="#1558D6"/>
        )}
        ...
        {score!=null
          ? <button className="btn-primary" style={{marginTop:16}} onClick={()=>{stop();navigate('/check/step5')}} >Continue →</button>
          : <button className="btn-outline" style={{marginTop:16}} onClick={()=>{stop();navigate('/check/step5')}} >Skip this step</button>
        }
      </div>
